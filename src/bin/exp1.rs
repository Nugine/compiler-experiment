use compiler_experiment::exp1::errors::SynError;
use compiler_experiment::exp1::lexer::Lexer;
use compiler_experiment::exp1::tokens::{self, Token};
use compiler_experiment::source_file::SourceFile;
use compiler_experiment::utils::number_width;
...
fn eprint_error_span(error: &SynError, src_lines: &[Vec<char>]) {
    ...
    if start_lc.line == end_lc.line {
        ...
        eprintln!(
            " --> {}:{}:{}",
            error.span.file_path.as_ref().unwrap(),
            start_lc.line,
            start_lc.column
        );
        eprintln!("{}", line_str);
